import os
import sys
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "world_cup_predictions.settings")
...
groups = {'Russia':1, 'Saudi Arabia':1, 'Egypt':1, 'Uruguay':1, 
          ...
          'France':3, 'Australia':3, 'Peru':3, 'Denmark':3,
          'Argentina':4, 'Iceland':4, 'Croatia':4, 'Nigeria':4,
          'Brazil':5, 'Switzerland':5, 'Costa Rica':5, 'Serbia':5,
          'Germany':6, 'Mexico':6, 'Sweden':6, 'Korea Republic':6,
          'Belgium':7, 'Panama':7, 'Tunisia':7, 'England':7,
          'Poland':8, 'Senegal':8, 'Colombia':8, 'Japan':8
          }
...
def populate_historical_matches(matches):
  for match in matches.itertuples(index=True, name='Pandas'):
    game = HistoricalGame(home_team=getattr(match,'home_team'),away_team=getattr(match,'away_team'),home_score=getattr(match,'home_score'),away_score=getattr(match,'away_score'))
    game.save()
